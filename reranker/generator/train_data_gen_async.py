...
class generate_openai():
    ...
    def generate(self):
        ...
        output_file = file_name = f"{self.config['generator']['output_dir']}/{self.config['seed']}_{self.config['mode']}_full_0731.jsonl"
    ...
    def get_data(self, config):
        ...
        #     user_id_path = "data-new/LaMP/{}/{}/{}_questions_extended.json".format(config['task'], config['mode'], config['mode'])
        # else:
        #     user_id_path = "data-new/LaMP/{}/{}/{}_query_questions_extended.json".format(config['task'], config['mode'], config['mode'])
        ...
        if config['mode'] == 'train':
            file_path = "data-new/LaMP/{}/{}/{}_rankings_extended.json".format(config['task'], config['mode'], config['mode'])
        else:
            ...
            if user['id'] in user_id_list: # and (user['id'] not in existing_ids):
                ...
                for profile in user['profile']:
                    if profile['id'] in ranking:
                        if config['task'] == 'LaMP_2':
                            reranker_data.append({'id': user['id'], 'pid': profile['id'], 'source': user['input'], 'target': solution_dict[user['id']], 'retrieval': profile['text'], 'retr_ans': profile['category']})
                        elif config['task'] == 'LaMP_2_movie':
                            reranker_data.append({'id': user['id'], 'pid': profile['id'], 'source': user['input'], 'target': solution_dict[user['id']], 'retrieval': profile['description'], 'retr_ans': profile['tag']})
                        elif config['task'] == 'LaMP_3':
                            reranker_data.append({'id': user['id'], 'pid': profile['id'], 'source': user['input'], 'target': solution_dict[user['id']], 'retrieval': profile['text'], 'retr_ans': profile['score']})
                        elif config['task'] == 'LaMP_4':
                            reranker_data.append({'id': user['id'], 'pid': profile['id'], 'source': user['input'], 'target': solution_dict[user['id']], 'retrieval': profile['text'], 'retr_ans': profile['title']})
                        elif config['task'] == 'LaMP_5':
                            reranker_data.append({'id': user['id'], 'pid': profile['id'], 'source': user['input'], 'target': solution_dict[user['id']], 'retrieval': profile['abstract'], 'retr_ans': profile['title']})
                ...
                for profile in profile_data:
                    ...
                    for profile2 in user['profile']:
                        if profile2['id'] in profile_ranking:
                            if config['task'] == 'LaMP_2':
                                reranker_data.append({'id': user['id'], 'pid': profile['id'], 'source': profile['text'], 'target': profile['category'], 'retrieval': profile2['text'], 'retr_ans': profile2['category']})
                            elif config['task'] == 'LaMP_2_movie':
                                reranker_data.append({'id': user['id'], 'pid': profile['id'], 'source': profile['description'], 'target': profile['tag'], 'retrieval': profile2['description'], 'retr_ans': profile2['tag']})
                            elif config['task'] == 'LaMP_3':
                                reranker_data.append({'id': user['id'], 'pid': profile['id'], 'source': profile['text'], 'target': profile['score'], 'retrieval': profile2['text'], 'retr_ans': profile2['score']})
                            elif config['task'] == 'LaMP_4':
                                reranker_data.append({'id': user['id'], 'pid': profile['id'], 'source': profile['text'], 'target': profile['title'], 'retrieval': profile2['text'], 'retr_ans': profile2['title']})
                            elif config['task'] == 'LaMP_5':
                                reranker_data.append({'id': user['id'], 'pid': profile['id'], 'source': profile['abstract'], 'target': profile['title'], 'retrieval': profile2['abstract'], 'retr_ans': profile2['title']})
        print(len(reranker_data))
